...
def inited_db(request, empty_db):
    empty_db['conn'].execute('CREATE TABLE jobs('
                             '    job_id CHARACTER(32),'
                             '    name VARCHAR(255),'
    ...
    empty_db['conn'].execute('CREATE TABLE IF NOT EXISTS job_log('
                             '   job_id CHARACTER(32),'
                             '   level INTEGER,'
                             '   time INT8,'
                             '   message TEXT'
...
def test_create_store(db_name):
    SQLiteJobStore(db_name)


def test_open_existing_store(empty_db):
    SQLiteJobStore(empty_db['file'])
...
def test_open_existing_store_data(onejob_db):
    ...
    with store:
        assert store.get_job(
            '258685677b034756b55bbad161b2b89b').name == 'test_sqlite_job_store'
...
def test_create_job(onejob_store):
    ...
    res = onejob_store['conn'].execute(
        """SELECT * FROM jobs WHERE name = 'test_create_job';""")
...
def test_reading_please_delete(job):
    assert job.please_delete is False


def test_setting_please_delete(job):
    job.please_delete = True
    assert job.please_delete is True
